...
df3 <- merge(df3, df_tmp, by='Country', all.x=TRUE) %>% select(c('Country','Population','Growth_Rate')) %>% arrange(desc(Population))
df3
```
...
df4 <- df %>% arrange(Density) %>% slice(1:5) %>% select(c("Country","Population_2022", "Density"))
df4
```
...
df5 <- df %>% arrange(Density) %>% slice(1:5) %>% select(c("Country", "Density", "Growth_Rate"))
df5
```
...
## Repeated cross-validation for preparing and running the model

After many considerations, I decided to perform repeated cross-validation on the following four models.
